    ...
    if (m_Socket->GetConnected())
    {
                ...
                && getElapsedMilliseconds() - m_LastOutPacketTicks >= WaitTicks)
        {
            if (m_OutPackets.size() > 7)
                CONSOLE_Print(ColoredMessage("[BNET] packet queue warning - there are " + QString::number(m_OutPackets.size()) + " packets waiting to be sent", ColoredMessage::BNET));
            ...
            || Event == CBNETProtocol::EID_JOIN)
    {
        ...
        for (int i = 0; i < statStringList.count(); i++)
        {
            switch (i)
            {
                case 0:
                {
                    QByteArray ba = statStringList.at(i).toUtf8();
                    char *d = ba.data();
